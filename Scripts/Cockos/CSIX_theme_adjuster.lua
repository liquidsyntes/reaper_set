-- Theme adjuster edition for CSIX themes
-- Theme adjuster edition for CSIX themes
-- note1: requires installation of image resources and font
-- note2: no 75/150pct scaling
-- Changelog:
-- 1.00  |  release
-- 1.01  |  update (hidpi ▶︎2x_)
-- 1.02  |  update (more coloring options)
-- 1.03  |  update (tcp sendlist, mixer channel strip, mcp fxembed)
-- 1.04  |  update (tcp: adjusted auto name-size, mcp: options for parm/sends, transport: toggle external timecode synchronization)
-- 1.05  |  update (added master mixer fxembed size)
-- 1.06  |  update (tcp: added button to show/hide fx inserts)
-- 1.07  |  update (adjusted font size, changed buttons style)
-- 1.08  |  update (tcp: meterflip opt., tcp: show-hide polarity, tcp: sepSends opt., tcp: custom button opt.)
-- 2.00  |  update
-- 3.00  |  update for CSIX v3
-- 3.01  |  update for CSIX v3.0.1
-- 3.02  |  update for CSIX v3.1.0 AE
-- 3.03  |  update for CSIX v3.1.1 AE
-- 3.04  |  update for CSIX Series v3.1.1
-------------------------------------------------------
...
function paramIdxGet(param)
  ...
  if param == 'tcp_indent' or param == 'tcp_control_align' or param == 'tcp_customButton' or param == 'tcp_indentGuide' or param == 'tcp_fxParmVis'
        or param == 'mcp_indent' or param == 'mcp_control_align' or param == 'mcp_customButton'
        or param == 'mcp_fxEmbedSizeMain' or param == 'tcp_LabelMeasure' or param == 'glb_TitleTrackSize' or param == 'glb_TitleTrackCol'
...
tcpVolVals = {'KNOB',40,70,100,130,160,170} --- CSIX (colorstrip)
--- tcpMeterVals = {4,10,20,60,100,160,320} ORG
tcpMeterVals = {'HIDE',1,2,3,4,5,6,7,8,'MAX'} --- CSIX (extra metersize)
...
tcpCustomButtonVals = {'w < 400','w < 500','w < 600','ALWAYS','w < 210 (AE)','w < 400 (AE)','w < 500 (AE)'} --- CSIX (tcp hide custom button)
tcpParmVisVals = {'Show','Show if Sel.','Hide'} --- CSIX (show/hide fx-parm)
...
mcpTitleTrackSize = {30,40,50,60,70,80,90,100} --- CSIX (mcp title-track)
mcpTitleTrackCol = {'Title','Panel','Neutral'} --- CSIX (tcp-mcp title-track)
mcpStripVolVals = {'MIN','+75','+110','+270','+370'} --- CSIX (strips volume size)
mcpStripNameVals = {40,60,80,110,170} --- CSIX (strips name size)
mcpStripExtMixVals = {'HIDE','SHOW'} --- CSIX (mcp strips hide extended mixer)
mcpFxEmbedVals = {'DEFAULT',100,200,300,400,500,'600 (MAX)'} --- CSIX (mcp fxembed size)
mcpFxEmbedMainVals = {'DEFAULT',150,200,300,400,500,'600 (MAX)'} --- CSIX (mcp master fxembed size)
trackLabelColVals = {'Default','rec.arm = RED','folder-track = BLUE'} --- CSIX (tcp custom overlay)
tcpCustomColorVals = {'0 %','20 %','40 %','60 %','80 %'} --- CSIX (tcp custom overlay)
mcpCustomButtonVals = {'HIDE ALL','SHOW','SHOW !','MAST. ONLY','ALL (AE)','ALL ! (AE)','MAST. ONLY (AE)'} --- CSIX (mcp custom buttons)
...
helpR_help = 'CSIX Theme Adjuster 3.04'
...
helpR_custOverlay = 'Adds an extra layer of custom color to all control panels (except on alternate full-tone layouts). The feature requires REAPER 7.'
...
helpR_showTcpParm = 'Reduce noise in TCP by only showing active Fx Parameter slots if track is selected (OFF). *Envelopes are not affected.'
helpR_showTcpSends = 'Show/Hide Sends below Insert in TCP.  '
                    ..' *Optionally engage \'SEP. SENDS\' (Separated list on the right). '
helpR_showActBtnMcp = 'Allow action buttons to be displayed on selected tracks and master mixer panel (when size permits). The feature requires REAPER 7.'
helpR_showIndentGuide = 'Displays folder indentation guides with a maximum depth of eight levels.  '
                    ..'*The feature is currently configured to work only with themes in CSIX BC and AE Series. Requires REAPER 7.'
...
helpR_extmix = 'When HIDE is selected, the VU meter fills the available space above the control panel. '
                ..'Selecting SHOW draws the Extended Mixer above the VU (when size permits). '
                ..'The Extended Mixer will always be drawn on the left side when Sidebar is visible.  '
                ..'*Only for slim Channel-Strips (layout C in CSIX BC, MC, DM and AE Series).'
...
--- apply.tcp[2] = Button:new(_applyBoxTcp, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'tcp','150%_'}}) --- CSIX (no dpi scale)
...
--- apply.mcp[2] = Button:new(_applyBoxMcp, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'mcp','150%_'}}) --- CSIX (no dpi scale)
...
--- apply.envcp[2] = Button:new(_applyBoxEnv, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'envcp','150%_'}}) --- CSIX (no dpi scale)
...
--- apply.trans[2] = Button:new(_applyBoxTrans, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'trans','150%_'},helpL=helpL_applySize}) --- CSIX (no dpi scale)
...
Button:new(_pageGlobal_und, {flow=false,x=150,y=600,w=205,h=40,img='themelogo',imgType=4}) --- CSIX theme logo
...
_colAdjBoxStroke = Element:new(_pageGlobal_und, {x=0,y=30,w=513,h=545,color={116,128,149,225}}) -- stroke
_colAdjBox = Element:new(_colAdjBoxStroke, {x=1,y=1,w=511,h=543,color={50,51,55},helpL=helpL_layout}) -- fill
Element:new(_colAdjBox, {x=0,y=26,w=511,h=16,text={str='COLOR CONTROLS',style=4,align=5,col={180,180,181}},helpR=helpR_trackLabelColor})
...
Button:new(_colAdjBox, {x=60,y=346,w=30,img='color_apply_all',imgType=3,action=paramToggle,param=-1006})
Element:new(_colAdjBox, {x=99,y=346,w=200,h=30,text={str='Also affect project Custom Colors',style=2,align=4,col={180,180,181}}})
Button:new(_colAdjBox, {flow=false,x=290,y=346,w=30,img='bin',imgType=3,action=resetColorControls,helpR=helpR_resetColAdj})
Element:new(_colAdjBox, {x=329,y=346,w=165,h=28,text={str='RESET all Color Controls',style=2,align=4,col={180,180,181}},helpR=helpR_resetColAdj})

Element:new(_colAdjBox, {x=30,y=398,w=450,h=1,color={116,128,149,225}}) -- title div
Element:new(_colAdjBox, {x=0,y=408,w=511,h=20,text={str='Tweaks for Basic Color Themes (only)',style=4,align=5,col={237,114,88}}})

Element:new(_colAdjBox, {x=7,y=442,w=255,h=16,text={str='TRACK NAME APPEARANCE',style=2,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=7,y=460,w=255,h=16,text={str='Indicates the track properties',style=2,align=5,col={179,197,229}}})
Spinner:new(_colAdjBox, {x=45,y=482,w=179,title='SHOW',action=paramSet,param='glb_trackLabelCol',valsTable=trackLabelColVals}) --- CSIX

Element:new(_colAdjBox, {x=247,y=442,w=255,h=16,text={str='TCP/MCP CUSTOM OVERLAY',style=2,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=247,y=460,w=255,h=16,text={str='* Please see Help text >',style=2,align=5,col={179,197,229}}})
Spinner:new(_colAdjBox, {x=285,y=482,w=179,title='COVERAGE',action=paramSet,param='glb_customOverlay',valsTable=tcpCustomColorVals}) --- CSIX

Element:new(_colAdjBox, {x=7,y=425,w=255,h=100,color={253,253,253,0},helpL=helpL_layout}) -- helpL
Element:new(_colAdjBox, {x=284,y=425,w=230,h=100,color={253,253,253,0},helpL=helpL_layout,helpR=helpR_custOverlay}) -- helpL, helpR
...
Button:new(_pageTrack_und, {x=261,y=39,w=30,img='Check-buttons_check',imgType=3,action=actionToggle,param=40302,helpR=helpR_showTcpFx,helpL=helpL_layout}) --- CSIX
Element:new(_pageTrack_und, {x=284,y=40,w=25,h=30,text={str='FX',style=2,align=4,col={180,180,181}},helpR=helpR_showTcpFx,helpL=helpL_layout})
...
Button:new(_pageTrack_und, {x=442,y=39,w=30,img='Check-buttons_check',imgType=3,action=actionToggle,param=40677,helpR=helpR_showTcpSends,helpL=helpL_layout}) --- CSIX
Element:new(_pageTrack_und, {x=465,y=40,w=50,h=30,text={str='SENDS',style=2,align=4,col={180,180,181}},helpR=helpR_showTcpSends,helpL=helpL_layout})
...
Readout:new(_tcpButLayA, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,60},colTrue={179,197,229}},updateState=anySelected,getParam={'tcp','A'},helpR=helpR_selected})
Readout:new(_tcpButLayB, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,60},colTrue={179,197,229}},updateState=anySelected,getParam={'tcp','B'},helpR=helpR_selected})
Readout:new(_tcpButLayC, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,60},colTrue={179,197,229}},updateState=anySelected,getParam={'tcp','C'},helpR=helpR_selected})
...
Spinner:new(_layoutTrack, {x=30,y=9,w=129,title='NAME SIZE',action=paramSet,param='tcp_LabelSize',valsTable=tcpLabelVals})
...
Spinner:new(_layoutTrack, {x=353,y=82,w=129,title='not used'}) --- CSIX (not used)
...
_trackTable = ParamTable:new(_layoutTrack, {x=29,y=125,w=453,h=355,valsTable=tcpTableVals})

--MIXER PAGE
_pageMixer_und = Element:new(_subPageContainer, {x=0,y=0,w=513,h=639})
Spinner:new(_pageMixer_und, {x=10,y=19,w=155,title='FOLDER INDENT',action=paramSet,param='mcp_indent',valsTable=folderIndentVals,helpR=helpR_indent,helpL=helpL_layout})
Spinner:new(_pageMixer_und, {x=180,y=19,w=155,title='ALIGN CONTROLS',action=paramSet,param='mcp_control_align',valsTable=controlAlignVals,helpR=helpR_control_align,helpL=helpL_layout})
Spinner:new(_pageMixer_und, {x=348,y=19,w=155,title='MCP ACTIONS *',action=paramSet,param='mcp_customButton',valsTable=mcpCustomButtonVals,helpL=helpL_layout,helpR=helpR_showActBtnMcp}) --- CSIX

_mixerTopStroke = Element:new(_pageMixer_und, {x=0,y=153,w=513,h=285,color={116,128,149,225}}) -- stroke
_mcpButLayA = Button:new(_pageMixer_und, {x=30,y=65,w=69,img='layout_docked_A_blue',imgType=3,action=doActiveLayout,param={'mcp','A'},helpR=helpR_layoutButton})
_mcpButLayB = Button:new(_pageMixer_und, {x=103,y=65,w=69,img='layout_docked_B_blue',imgType=3,action=doActiveLayout,param={'mcp','B'},helpR=helpR_layoutButton})
_mcpButLayC = Button:new(_pageMixer_und, {x=176,y=65,w=69,img='layout_docked_C_blue',imgType=3,action=doActiveLayout,param={'mcp','C'},helpR=helpR_layoutButton})
...
Readout:new(_mcpButLayA, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,60},colTrue={179,197,229}},updateState=anySelected,getParam={'mcp','A'},helpR=helpR_selected})
Readout:new(_mcpButLayB, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,60},colTrue={179,197,229}},updateState=anySelected,getParam={'mcp','B'},helpR=helpR_selected})
Readout:new(_mcpButLayC, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,60},colTrue={179,197,229}},updateState=anySelected,getParam={'mcp','C'},helpR=helpR_selected})

_applyStroke_Mixer = Element:new(_pageMixer_und, {x=274,y=70,w=239,h=83,color={116,128,149,225},helpL=helpL_applySize}) -- stroke
...
apply.und.mcp[2] = Button:new(_applyBox_Mixer, {x=1090,y=25,w=61,img='apply_150',imgType=3,action=applyLayout,param={'mcp','150%_'},helpR=helpR_applySize,helpL=helpL_applySize}) --- CSIX (no dpi scale) ORG x=90,y=29,w=61
apply.und.mcp[3] = Button:new(_applyBox_Mixer, {x=115,y=25,w=61,img='apply_200',imgType=3,action=applyLayout,param={'mcp','200%_'},helpR=helpR_applySize,helpL=helpL_applySize}) --- CSIX (no dpi scale) ORG 151,y=29,w=61
Element:new(_applyBox_Mixer, {x=17,y=60,w=200,h=10,img='apply_to_sel'})

_mixerTop = Element:new(_mixerTopStroke, {x=1,y=1,w=511,h=283,color={58,60,64},helpL=helpL_layout}) -- fill
...
Spinner:new(_mixerTop, {x=10,y=52,w=185,title='FX EMBED (SIDEBAR)',action=paramSet,param='mcp_fxEmbedSize',valsTable=mcpFxEmbedVals,helpL=helpL_layout,helpR=helpR_embed}) --- CSIX (mcp fxembed size)
Spinner:new(_mixerTop, {x=10,y=94,w=185,title='FX EMBED (MASTER WS)',action=paramSet,param='mcp_fxEmbedSizeMain',valsTable=mcpFxEmbedMainVals,helpL=helpL_layout,helpR=helpR_embedMast}) --- CSIX (mcp master fxembed size)
...
Spinner:new(_mixerTop, {x=208,y=52,w=140,title='Title-Track Size*',action=paramSet,param='glb_TitleTrackSize',valsTable=mcpTitleTrackSize,helpR=helpR_titleTrack1}) --- CSIX (mcp title-track)
Spinner:new(_mixerTop, {x=208,y=94,w=140,title='Title-Track Col.*',action=paramSet,param='glb_TitleTrackCol',valsTable=mcpTitleTrackCol,helpR=helpR_titleTrack2}) --- CSIX (tcp-mcp title-track)
...
Spinner:new(_mixerTop, {x=360,y=52,w=140,title='NAME SIZE (C)',action=paramSet,param='mcp_StripNameSize',valsTable=mcpStripNameVals}) --- CSIX (strips name size)
Spinner:new(_mixerTop, {x=360,y=94,w=140,title='EXT.MIXER (C) *',action=paramSet,param='mcp_StripExtMix',valsTable=mcpStripExtMixVals,helpL=helpL_layout,helpR=helpR_extmix}) --- CSIX (mcp strips hide extended mixer)
...
_mLower = Element:new(_pageMixer_und, {x=0,y=457,w=513,h=192,color={116,128,149,225}}) -- stroke
_extMixerf = Element:new(_mLower, {x=1,y=1,w=255,h=190,color={50,51,55},helpL=help_pref}) -- fill
Element:new(_extMixerf, {x=29,y=9,w=190,h=30,text={str='Extended Mixer Controls#when size permits',style=2,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=23,y=45,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40549}) --- CSIX
Element:new(_extMixerf, {x=50,y=45,w=190,h=30,text={str='Show FX Inserts',style=2,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=23,y=78,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40910}) --- CSIX
Element:new(_extMixerf, {x=50,y=78,w=190,h=30,text={str='Show FX Parameters',style=2,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=48,y=98,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=41829}) --- CSIX - group fx parm
Element:new(_extMixerf, {x=74,y=98,w=190,h=30,text={str='Group with their Inserts',style=2,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=23,y=126,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40557}) --- CSIX
Element:new(_extMixerf, {x=50,y=126,w=190,h=30,text={str='Show Sends',style=2,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=48,y=146,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40267}) --- CSIX - group sends
Element:new(_extMixerf, {x=74,y=146,w=190,h=30,text={str='Group below/after Inserts',style=2,align=4,col={180,180,181}}})

_parmMixer = Element:new(_mLower, {x=256,y=1,w=256,h=190,color={58,60,64},helpL=help_pref}) -- fill
Button:new(_parmMixer, {x=29,y=9,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40371}) --- CSIX
Element:new(_parmMixer, {x=58,y=9,w=190,h=30,text={str='Show Multiple-Row Mixer#when size permits',style=2,align=4,col={180,180,181}}})
Button:new(_parmMixer, {x=29,y=45,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40221}) --- CSIX
Element:new(_parmMixer, {x=58,y=45,w=190,h=30,text={str='Scroll to Selected Track',style=2,align=4,col={180,180,181}}})
Button:new(_parmMixer, {x=29,y=78,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40903}) --- CSIX
Element:new(_parmMixer, {x=58,y=78,w=190,h=30,text={str='Show Icons',style=2,align=4,col={180,180,181}}})
Button:new(_parmMixer, {x=29,y=111,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40389}) --- CSIX
Element:new(_parmMixer, {x=58,y=111,w=190,h=30,text={str='Show Master on right side',style=2,align=4,col={180,180,181}}})
Button:new(_parmMixer, {x=54,y=131,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=41610}) --- CSIX
Element:new(_parmMixer, {x=78,y=131,w=190,h=30,text={str='Toggle in dock',style=2,align=4,col={180,180,181}}})
Button:new(_parmMixer, {x=54,y=151,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=41636}) --- CSIX
Element:new(_parmMixer, {x=78,y=151,w=190,h=30,text={str='Toggle in separate window',style=2,align=4,col={180,180,181}}})


--COLOR PAGE
_pageColor_und = Element:new(_subPageContainer, {x=0,y=40,w=513,h=639})
_paletteBoxStroke = Element:new(_pageColor_und, {x=0,y=0,w=513,h=475,color={116,128,149,225}}) -- stroke
_paletteBox = Element:new(_paletteBoxStroke, {x=1,y=1,w=511,h=205,color={58,60,64},helpL=helpL_customCol}) -- fill
...
-- Element:new(_paletteBox, {x=179,y=145,w=208,h=30,text={str='Recolor project using this palette',style=2,align=4,col={180,180,181}},helpR=helpR_recolProject})
...
Element:new(_paletteBox, {x=179-112,y=145,w=208,h=30,text={str='Project: Recolor using this palette',style=2,align=4,col={180,180,181}},helpR=helpR_recolProject})
...
Element:new(_paletteBox, {x=315,y=135,w=208,h=30,text={str='Items: Set to Default color',style=2,align=4,col={180,180,181}}})
...
Element:new(_paletteBox, {x=315,y=165,w=208,h=30,text={str='Takes: Set to Default color',style=2,align=4,col={180,180,181}}})

_paletteMenuBox = Element:new(_paletteBoxStroke, {x=1,y=202,w=511,h=272,color={50,51,55}}) -- fill
...
Element:new(_pageColor_und, {x=67,y=492,w=208,h=30,text={str='Dim all Assigned custom colors',style=2,align=4,col={180,180,181}},helpR=helpL_colDimming})
...
Element:new(_pageColor_und, {x=67,y=535,w=208,h=30,text={str='Dim custom colors on Selected tracks',style=2,align=4,col={180,180,181}},helpR=helpL_colDimming})

Button:new(_pageColor_und, {x=28,y=579,w=30,img='bin',imgType=3,action=setTrackDefaultColor})
Element:new(_pageColor_und, {x=67,y=578,w=208,h=30,text={str='RESET selected Tracks to Default color',style=2,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=278,y=492,w=30,img='color_dim',imgType=3,action=TrackRandomColor})
Element:new(_pageColor_und, {x=315,y=492,w=208,h=30,text={str='Track: Set to Random colors (native)',style=2,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=278,y=535,w=30,img='color_dim',imgType=3,action=TrackCustomColor})
Element:new(_pageColor_und, {x=315,y=535,w=208,h=30,text={str='Track: Set to Custom color (picker)',style=2,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=278,y=578,w=30,img='toolbar_colorpal',imgType=3,action=CustomColorScript})
Element:new(_pageColor_und, {x=315,y=578,w=208,h=30,text={str='Rodilab Color palette (ReaPack add-on)',style=2,align=4,col={180,180,181}}})
...
apply.und.envcp[2] = Button:new(_pageEnvTrans_und, {x=1228,y=2,w=61,img='apply_150',imgType=3,action=applyLayout,param={'envcp','150%_'},helpL=helpL_applySize}) --- CSIX (no dpi scale) ORG x=228,y=2,w=61
...
Button:new(_pageEnvTrans_und, {flow=false,x=150,y=104,w=30,img='Check-buttons_check',imgType=3,action=paramToggle,param='envcp_folder_indent',helpR=helpR_emvMatchIndent}) --- CSIX
...
apply.und.trans[2] = Button:new(_pageEnvTrans_und, {x=1228,y=278,w=61,img='apply_150',imgType=3,action=applyLayout,param={'trans','150%_'},helpL=helpL_applySize}) --- CSIX (no dpi scale) ORG x=228,y=2,w=61
...
Element:new(_pageEnvTrans_und, {x=154,y=322,w=200,h=16,text={str='* Please see Help text > > >',style=2,align=5,col={179,197,229}}}) --- CSIX
...
Button:new(_transPrefs, {x=29,y=26,img='Check-buttons_check',imgType=3,action=actionToggle,param=40533}) --- CSIX
Element:new(_transPrefs, {x=68,y=26,w=150,h=30,text={str='Center Transport',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=29,y=69,img='Check-buttons_check',imgType=3,action=actionToggle,param=40531}) --- CSIX
Element:new(_transPrefs, {x=68,y=69,w=150,h=30,text={str='Show Play Rate',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=29,y=112,img='Check-buttons_check',imgType=3,action=actionToggle,param=40680}) --- CSIX
Element:new(_transPrefs, {x=68,y=112,w=170,h=30,text={str='Show Time Signature',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=26,img='Check-buttons_check',imgType=3,action=actionToggle,param=40868}) --- CSIX
Element:new(_transPrefs, {x=324,y=26,w=170,h=30,text={str='Use Home/End for Markers',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=69,img='Check-buttons_check',imgType=3,action=actionToggle,param=40532}) --- CSIX
Element:new(_transPrefs, {x=324,y=69,w=170,h=30,text={str='Show Play State as Text',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=112,img='Check-buttons_check',imgType=3,action=actionToggle,param=40620}) --- CSIX
